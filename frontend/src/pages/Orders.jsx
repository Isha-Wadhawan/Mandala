  ...
  const loadOrderData = async () => {
    try {
      ...
      const response = await axios.get(backendUrl + '/api/order/userorders', { headers: { token: token } })
              console.log("2. Full Orders Array:", response.data);
            ...
            <div key={index} className='py-4 border-t border-b text-gray-700 flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
              <div className='flex items-start gap-6 text-sm'>
                <img
                  ...
                  src={Array.isArray(item.image) ? item.image[0] : (Array.isArray(item.images) ? item.images[0] : (item.image || item.images))}
                  alt="Product"
                />
                <div>
                  ...
                  <p className='mt-2'>Payment : <span className='text-gray-400'>{item.payment ? 'Paid' : 'Pending Verification'}</span></p>
                </div>
              </div>
